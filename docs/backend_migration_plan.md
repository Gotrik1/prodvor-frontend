...
5.  **`GET /api/v1/users`**:
    -   Должен возвращать пагинированный список всех пользователей.
...
9.  **`POST /api/v1/uploads/initiate`**
    -   **Описание:** Создает presigned URL для прямой загрузки файла в S3.
    -   **Защита:** Требует аутентификации.
    -   **Тело запроса:** `{"contentType": "image/jpeg", "purpose": "avatar"}`.
    -   **Логика:**
        1.  Инициализировать MinIO клиент.
        2.  Сгенерировать уникальное имя файла.
        3.  Создать политику загрузки.
        4.  Вернуть ответ в формате `{"url": ..., "fields": {...}, "fileId": "..."}`.

10. **`POST /api/v1/uploads/complete`**
    -   **Описание:** Связывает загруженный файл с сущностью.
    -   **Защита:** Требует аутентификации.
    -   **Тело запроса:** `{"fileId": "...", "entityId": "user-123", "entityType": "user_avatar"}`.
    -   **Логика:** Найти пользователя по `entityId` и обновить его поле `avatarUrl`.
...
1.  **`GET /api/v1/teams`**:
    -   Должен возвращать пагинированный список всех команд.
...
2.  **`GET /api/v1/teams/{team_id}/applications`**
    -   Требует аутентификации. Доступен только капитану команды.
    -   Возвращает пагинированный список полных объектов пользователей, подавших заявки со статусом `pending`.
...
4.  **`GET /api/v1/sports`**: Возвращает пагинированный список всех видов спорта из таблицы `Sport`.

5.  **`GET /api/v1/playgrounds`**: Возвращает пагинированный список всех площадок.

6.  **`POST /api/v1/playgrounds`**: Создает новую площадку.

7.  **`GET /api/v1/tournaments`**: Возвращает пагинированный список всех турниров.
...
10. **`GET /api/v1/posts`**: Возвращает пагинированный список всех постов.

11. **`POST /api/v1/posts`**: Создает новый пост.

12. **`GET /api/v1/sponsors`**: Возвращает пагинированный список всех спонсоров.
"
---
