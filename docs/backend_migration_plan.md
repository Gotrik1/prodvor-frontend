...
2.  **`GET /api/v1/teams/<int:team_id>`**:
    ...
    -   **КРИТИЧЕСКИ ВАЖНО:** Ответ должен содержать объект капитана в поле `captain` и массив объектов участников в поле `members`. **Объект капитана также должен быть включен в массив `members`**. Это упростит обработку на фронтенде.
...
4. **`DELETE /api/v1/teams/<int:team_id>/members/<int:user_id>`**:
    -   **Описание:** Удаление игрока из состава команды.
    -   **Защита:** Требует аутентификации (`@jwt_required()`). Доступен **только капитану команды** (`team_id`).
    -   **Логика:**
        1.  Проверить, что текущий пользователь (из JWT-токена) является капитаном команды `team_id`. Если нет — вернуть ошибку 403 (Forbidden).
        2.  Проверить, что `user_id` (удаляемый игрок) не является капитаном. Капитан не может исключить сам себя. Если это так — вернуть ошибку 400 (Bad Request).
        3.  Удалить соответствующую запись из связующей таблицы `TeamMembers`.
    -   **Ответ в случае успеха (200 OK):** `{\"success\": true, \"message\": \"Player removed successfully\"}`."
