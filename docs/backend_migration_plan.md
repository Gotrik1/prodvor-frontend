...
2.  **`GET /api/v1/teams/<int:team_id>`**:
    -   Должен находить команду по `team_id` и возвращать её полные данные, **включая список ID участников** (`members`).
...
## Шаг 7: Обновление данных пользователя (включая дисциплины)
