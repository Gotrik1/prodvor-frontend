...
Если после этого шага ошибка `Failed to fetch` или `Network Error` исчезнет, значит, проблема была именно в CORS. Позже вы сможете заменить `"*"` на точный URL вашего фронтенда для большей безопасности.
