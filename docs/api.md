...
**ВАЖНО:** Этот документ является первоначальным руководством. Основным и наиболее точным источником правды является **спецификация OpenAPI**, которая автоматически обновляется в файле `src/docs/openapi.json`. 

AI-ассистент **никогда не редактирует** `src/docs/openapi.json`. Он использует его только для чтения как единственный источник правды об API.
...
*   **`GET /users/<user_id>`**
    *   **Описание:** Возвращает информацию о конкретном пользователе. Можно добавить `?include_teams=true` для включения списка команд пользователя.
    *   **Ответ:** Объект пользователя.
...
*   **`GET /teams/<team_id>`**
    *   **Описание:** Возвращает информацию о конкретной команде, включая полные объекты капитана и участников.
    *   **Ответ:** Объект команды (см. пример в техническом задании).

*   **`POST /teams`**
    *   **Описание:** Создает новую команду. Капитан определяется автоматически по токену.
    *   **Тело запроса:**
        ```json
        {
...
*   **`DELETE /teams/<team_id>/members/<user_id>`**
    *   **Описание:** Удаляет игрока из команды. Только для капитана.
...
*   **`POST /teams/<team_id>/follow`**
    *   **Описание:** Подписаться/отписаться от команды. Требует аутентификации.
    *   **Ответ:**
        ```json
        {
          "success": true,
