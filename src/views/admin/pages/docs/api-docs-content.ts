...
- **GET \`/feed\`**: Получение персонализированной ленты новостей для текущего пользователя.
  - **Логика:** Требует авторизации. Бэкенд должен определить \`userId\` из токена, найти его друзей (из \`user.friends\`) и команды, на которые он подписан (\`user.followingTeams\`), а затем вернуть все посты, где \`authorId\` входит в список друзей или \`teamId\` входит в список отслеживаемых команд, отсортированные по дате.
