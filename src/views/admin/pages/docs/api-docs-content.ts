...
- **POST \`/tournaments/:id/matches/:matchId/score\`**: Внесение результата матча.
  - **Auth:** Требуется (для судьи или организатора).
  - **Примечание:** Этот эндпоинт должен запускать на бэкенде пересчет ELO-рейтинга для участвующих команд.
...
- **POST \`/ai/generate-logo\`**: Генерация логотипа для команды.
- **POST \`/ai/analyze-video\`**: Анализ видео матча.
- **GET \`/ai/news-digest\`**: Генерация новостного дайджеста.
- **POST \`/ai/ask-rules-expert\`**: Задать вопрос эксперту по правилам.
- **POST \`/ai/generate-tournament-image\`**: Генерация промо-изображения для турнира.
- **POST \`/ai/generate-tournament-promo\`**: Генерация промо-видео для турнира.

### 4.11. Admin & Dashboard Endpoints (\`/admin/...\`)

- **GET \`/admin/stats\`**: Получение агрегированной статистики для главной страницы админ-панели.
- **GET \`/admin/users\`**: Получение полного списка пользователей с фильтрацией и пагинацией.
