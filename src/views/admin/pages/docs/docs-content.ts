...
### Работа с AI-функциями (Моки)

- Все "умные" функции, такие как генерация логотипов или анализ матчей, на данном этапе разработки являются **моками (заглушками)**.
- Функции, имитирующие работу AI, находятся в директории \`src/shared/api/\`.
- Они не делают запросов к внешним сервисам, а возвращают заранее подготовленные (hardcoded) данные. Это позволяет разрабатывать и тестировать интерфейс без реального подключения к AI-моделям.
- Клиентские компоненты вызывают эти моковые функции через Server Actions, определенные в \`src/app/actions.ts\`.
