...
### 10.1. Admin Core & Simulation Service
...
Проект использует App Router из Next.js. Ключевые директории:

- **\`src/app/\`**: Содержит все маршруты, страницы и лейауты приложения.
  - **\`(main)/\`**: Группа маршрутов для аутентифицированных пользователей (дашборд, команды и т.д.).
  - **\`(public)/\`**: Группа маршрутов для публичных страниц (главная, о проекте, вход).
  - **\`admin/\`**: Панель администратора.
  - **\`api/\`**: API-роуты Next.js (используются для серверных действий).

- **\`src/views/\`**: Компоненты-страницы, разделенные по фичам (например, \`views/teams\`, \`views/tournaments\`). Этот подход позволяет изолировать логику отображения.

- **\`src/widgets/\`**: Более крупные, переиспользуемые блоки UI, состоящие из нескольких компонентов (например, \`widgets/dashboard-header\`, \`widgets/logo-generator\`).

- **\`src/entities/\`**: "Атомарные" компоненты, представляющие бизнес-сущности (например, \`entities/team/ui/TeamCard.tsx\`).

- **\`src/shared/\`**: Общий код, используемый во всем приложении.
  - **\`ui/\`**: Базовые UI-компоненты (Button, Card, Input и т.д.), в основном из ShadCN.
  - **\`lib/\`**: Общие утилиты и хелперы (например, \`cn\` для стилей).
  - **\`hooks/\`**: Глобальные хуки.
  - **\`api/\`**: Функции для взаимодействия с бэкендом (включая Server Actions).

- **\`src/mocks/\`**: Моковые (тестовые) данные для симуляции работы приложения без реального бэкенда. Файл \`initialize.ts\` отвечает за создание связей между разными сущностями (например, добавление игроков в команды).
