...
// Mock data for a specific referee or a generic one
const defaultReferee = staff.find(s => s.role === 'Судья')!;
const mockMatches = [
...
export function RefereePageTemplate({ user }: { user?: Staff }) {
        ...
        <div className="border rounded-lg p-4 md:p-6 space-y-6 bg-muted/20">
            <header className="flex flex-col md:flex-row items-center gap-6">
                <Avatar className="h-24 w-24 border-4 border-primary">
                    ...
                    <AvatarFallback>{referee.name.split(' ').map(n => n[0]).join('')}</AvatarFallback>
                </Avatar>
                <div className="text-center md:text-left">
                    <h1 className="text-3xl font-bold font-headline">{referee.name}</h1>
                    <p className="text-muted-foreground text-lg">Роль: Судья</p>
