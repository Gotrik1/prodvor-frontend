...
import { PlusCircle, Send, Gavel, ShieldAlert, Edit, Ban } from "lucide-react";
...
const mockDisqualifiedPlayers = [
    { ...users[0], status: 'Активна', reason: 'Красная карточка', duration: '1 матч' },
    { ...users[8], status: 'Завершена', reason: 'Перебор желтых', duration: '1 матч' },
];
            ...
            <Card>
                ...
                <CardContent>
                    <ul className="space-y-3">
                        {users.filter(s => s.role === 'Судья' || s.role === 'Организатор').slice(0, 3).map(person => (
                            <li key={person.id}>
                                        ...
                                        <TableCell>
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-xs text-muted-foreground">{player.reason} ({player.duration})</p>
                                                </div>
                                            </div>
                                        </TableCell>
                                        <TableCell>
                                            <Badge className={sanctionStatusColors[player.status]}>{player.status}</Badge>
                                        </TableCell>
                                        <TableCell className="text-right">
                                             <Dialog>
                                                <DialogTrigger asChild>
                                                    <Button variant="ghost" size="sm"><Edit className="mr-2 h-3 w-3" />Изменить</Button>
                                                </DialogTrigger>
                                                <DialogContent>
                                                    <DialogHeader>
                                                         ...
                                                         <div className="space-y-2">
                                                            ...
                                                            <Select defaultValue={player.status}>
                                                                ...
                                                                <SelectContent>
                                                                    ...
                                                                    <SelectItem value="Пожизненная">Пожизненная</SelectItem>
                                                                </SelectContent>
                                                            </Select>
                                                        </div>
                                                    </div>
