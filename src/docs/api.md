...
Этот документ описывает RESTful API, необходимое для поддержки функционала платформы ProDvor. 

**Важно:** Этот документ является первоначальным руководством. По мере развития бэкенда, основным и наиболее точным источником правды должна стать **спецификация OpenAPI (Swagger)**. Пожалуйста, сохраняйте актуальный файл `openapi.json` в директории `/src/docs/`, чтобы обеспечить синхронизацию между фронтендом и бэкендом.
