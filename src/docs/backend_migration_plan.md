...
-   **UserAchievement** (связующая таблица для достижений пользователя):
    -   `userId`: `Integer`, внешний ключ к `user.id`, первичный ключ
    -   `achievementId`: `Integer`, внешний ключ к `achievement.id`, первичный ключ
    -   `unlockedAt`: `DateTime`, по умолчанию `func.now()`
...
6.  **`GET /api/v1/users/me`**:
    ...
    -   Возвращает **полный объект пользователя**, включая `player_profile` и `unlockedAchievements`.

7.  **`GET /api/v1/users/:user_id`**:
    -   Находит пользователя по `user_id`.
    -   **Логика:**
        -   Если есть query-параметр `include_teams=true`, ответ **должен содержать** массив полных объектов команд пользователя в поле `teams`.
    -   Возвращает **полный объект пользователя**, включая `player_profile` и `unlockedAchievements`. Если не найден, ошибка 404.
