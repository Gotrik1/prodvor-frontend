...
## 7. Общие стандарты API (Дополнение)

Эти правила дополняют существующую архитектуру и применяются ко ВСЕМ эндпоинтам в проекте для обеспечения консистентности.

### 7.1. Стандартизация ответов с ошибками (4xx, 5xx)

Любая ошибка должна возвращаться в стандартизированном формате для предсказуемой обработки на клиенте.
...
```
- `code`: Машиночитаемый код ошибки (`validation_error`, `not_found`, `unauthorized`).
- `message`: Человекочитаемое описание ошибки.
- `details`: (Опционально) Объект с дополнительной информацией.

### 7.2. Пагинация

Параметры пагинации для вложенных ресурсов и всех остальных списков стандартизируются.
- `?page=N`: Номер запрашиваемой страницы (начиная с 1). По умолчанию: 1.
- `?limit=N`: Количество элементов на странице. По умолчанию: 20. Максимум: 100.

Ответ для пагинированного списка должен содержать блок `pagination` вместе с основными данными.

**Пример для списка с пагинацией (GET /api/v1/users/:id/friends):**
```json
{
  "friends": {
    "count": 54,
...
Фильтрация для списков реализуется через query-параметры. Имена параметров должны совпадать с именами полей.
**Пример:** `GET /api/v1/users?city=Москва&role=player&nickname_like=Ivan`

---

## 8. Политики доступа (RBAC - Role-Based Access Control)

Этот раздел формализует права доступа на основе ролей.

- **Администратор (`admin`):**
  - Полный доступ (CRUD) ко всем сущностям: пользователи, команды, турниры и т.д.
- **Пользователь (`player`, `coach`, etc.):**
  - Может просматривать и редактировать **только свой** профиль (`/users/me`) и свои настройки.
  - Может просматривать чужие профили и их разделы (`friends`, `teams`) в соответствии с **настройками приватности** этих профилей.
- **Капитан команды (`captain`):**
  - Может редактировать профиль своей команды.
  - Может управлять составом своей команды (принимать/исключать игроков).
- **Организатор турнира (`organizer`):**
  - Может создавать и управлять своими турнирами.
