...
- **Статус:** **Решено.**
- **Эндпоинт:** `POST /api/v1/login` и `GET /api/v1/users/me`.
- **Логика:**
    1. Фронтенд отправляет запрос на `POST /api/v1/login`.
    2. Бэкенд возвращает `accessToken` и `refreshToken`.
    3. Фронтенд сохраняет токены и **сразу делает** новый запрос на `GET /api/v1/users/me`, используя полученный `accessToken`.
    4. Бэкенд возвращает полный объект пользователя, который сохраняется в сторе.
- **Действие:** Эта двухэтапная логика обеспечивает безопасность и получение актуальных данных пользователя.
