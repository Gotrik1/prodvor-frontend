...
## 4. Соглашения по качеству кода

Для поддержания высокого качества кода и упрощения командной работы мы придерживаемся следующих **абсолютных правил**:

1.  **Рефакторинг и модульность**: Регулярно проводите рефакторинг. Разбивайте сложный функционал на небольшие, переиспользуемые модули. Это делает код более читаемым и легким для тестирования.
2.  **Ограничение размера файлов**: Каждый файл не должен превышать **300 строк кода**. Если файл разрастается, это прямой сигнал к его рефакторингу и разделению на более мелкие модули.
3.  **Комментирование**: Весь неочевидный код, сложная бизнес-логика или важные архитектурные решения должны сопровождаться комментариями. Объясняйте *почему* код написан так, а не *что* он делает.
